...
from chatglm import ChatGlm_inference
from collections import defaultdict
from ChatGPT import ChatGPT
from musachat import Musachat_inference
from utils import If_Skill, check, build_skill, guess_stop, get_api_key, post_process
...
class DmClass(object):
    def __init__(self, args):
        ...
        self.chatglm = ChatGlm_inference(self.args.GLM_ip, self.args.GLM_port)
        self.musachat = Musachat_inference(self.args.musachat_ip, self.args.musachat_port)
        ...
        self.max_tokens = self.args.musachat_max_tokens
        self.len_prompt = self.args.len_prompt
        self.chat_thres = 0.85
        self.qr_thres = 0.8
        self.faq_thres = 0.75
    ...
    def load_data(self, query, history, system_prompt):
        self.query = query
    ...
    def response(self):
        ...
        #FAQ+musachat
        elif self.args.musachat:
            messages = []
            # prompt.append({"role":"system", "content":'你是摩尔线程的智能对话数字人，名字叫做穆莎。你的对话能力来自摩尔线程自研的中英双语对话模型，采用了文本预训练、代码预训练、指令学习、强化学习和有监督对齐等技术。这使我具备丰富的技能，比如我可以使用外部工具来解决数值计算问题，调用马良系统来根据文字生成图片，还有回答知识问题和写代码的能力。这些最前沿的AI能力和数字人应用，是完整地运行在我们的全功能GPU之上的。你所在的空间， 是真实存在于摩尔北京总部的一个展厅 ，它是通过混合渲染管线制作和渲染的。我们只需要对着场景拍摄几分钟的视频，而后进行神经辐射场的训练，就可以通过AI全自动地复刻真实场景了。混合渲染技术打通了深度神经网络和经典渲染管线的壁垒，同时使用AI算力与图形算力，最大化发挥了全功能GPU的优势。'})
            if self.system_prompt:
                messages.append({'role': 'system', 'content': self.system_prompt})
            if len(self.conversation)>=2:
                for i in range(0, len(self.conversation),2):
                    messages.append({"role":"user", "content":self.conversation[i]})
                    messages.append({"role":"assistant", "content":self.conversation[i+1]})
            ...
            payload = {
                ...
                'stream': True}
            output = ''
            for answer in self.musachat.response(payload):
                yield answer
                output = answer
            logger.info('dm response :{} \n'.format(output))
            ...
            if self.args.FAQ:
                ...
                if score>=self.chat_thres:
                    ...
                    for answer in self.chatglm.response(list(self.history.conversation), self.query):
                        yield answer
            else:
                for answer in self.chatglm.response(list(self.history.conversation), self.query):
                    yield answer
